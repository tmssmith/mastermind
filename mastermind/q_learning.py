...
class QLearningAgent:
    ...
    def policy(self, state: Hashable, test: bool = False):
        ...
        max_q_actions = [action for action, value in self.q[state].items() if value == max_q_value]
        return random.choice(max_q_actions)

    def update_q_table(self, state: Hashable, action: tuple, reward, next_state: Hashable):
    ...
    def initialise_q(self, state):
        self.q[state] = {tuple(action): 0.0 for action in self.actions}  # initialise Q(S,:) with 0.0
